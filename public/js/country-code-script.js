...
class CCFEF extends elementorModules.frontend.handlers.Base {
  ...
  handleTelWithMdcFields(iti) {
    ...
    $inputs.eq(0).on("mouseover", () => {
      const borderWidth = getComputedStyle($notchedOutlineLeading[0]).getPropertyValue(
        "border-bottom-width"
      );
    ...
    $parent.on("mouseleave", (e) => {
      ...
      const borderColor = getComputedStyle($notchedOutlineLeading[0]).getPropertyValue(
        "border-right-color"
      );
      $notchedOutlineNotch.css({
  ...
  addCountryCodeIconHandler(formId, widgetId, inputId) {
    ...
    const telFIeld = jQuery(
      `.elementor-widget.elementor-widget-cool-form[data-id="${formId}"] .is-field-type-tel.cool-form__field-group input[type="tel"]#${inputId}`
    )[0];
    ...
    if (undefined !== telFIeld) {
      ...
      if (
        this.includeCountries.hasOwnProperty(uniqueId) &&
        this.includeCountries[uniqueId].length > 0
      ) {
      ...
      if (
        this.excludeCountries.hasOwnProperty(uniqueId) &&
        this.excludeCountries[uniqueId].length > 0
      ) {
        let uniqueValue = defaultCoutiresArr.filter(
          (value) => !this.excludeCountries[uniqueId].includes(value)
        );
      ...
      const intlSpan = document.querySelector(
        `.elementor-widget.elementor-widget-form[data-id="${formId}"] .ccfef-editor-intl-input[data-field-id="${widgetId}"]`
      );
      const commonAttr = intlSpan ? intlSpan.getAttribute("data-common-countries") : "";
      if (
        "same" === commonAttr &&
        this.commonCountries[uniqueId] &&
        "" !== includeCountries &&
        "" !== excludeCountries
      ) {
        ...
        if (countryList && countryList.classList.contains("iti__country-list")) {
          ...
          const visibleCountries = Array.from(countryItems).filter(
          ...
          if (includedVisibleCountries.length > 0) {
            const selectedItem = includedVisibleCountries.find(
  ...
  setInitialCountry(itiInstance, excludeCountries, uniqueId, telField) {
    ...
    if (
      jQuery(telField)
        ...
        .hasClass("elementor-field-required") &&
    ...
    const setCountry = (countryCode) => {
      ...
      const normalizedCountryCode =
        isNaN(countryCode) && countryCode ? countryCode.toLowerCase() : "";
      if (normalizedCountryCode && itiCountriesList.includes(normalizedCountryCode)) {
        ...
        const availableCountries = defaultCountries.filter(
          (country) => !excludeCountries.includes(country) && itiCountriesList.includes(country)
        );

        const fallbackCountry =
          availableCountries.length > 0 ? availableCountries[0] : itiCountriesList[0];
  ...
  customFlags() {
    ...
    selectedCountries.each(function () {
      ...
      if (classList[1]) {
        ...
        const svgFlagPath =
          CCFEFCustomData.pluginDir + `assets/addons/flags/${selectedCountryFlag}.svg`;
        ...
        if ("same" !== commonAttr && "" !== includeCountries && "" !== excludeCountries) {
          if (isNaN(includeCountries)) {
            ...
            if (isNaN(excludeCountries)) {
              ...
              const allIncludedPresent = includeArray.every((country) =>
                excludeArray.includes(country)
              );
