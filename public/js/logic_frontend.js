  ...
  $(document).ready(function () {
        ...
        try {
          ...
          $.each(logicData, function (logic_key, logic_value) {
            ...
            if ($(form).find(".is-field-group-" + logic_key).length) {
              field = $(form)
                .find(".is-field-group-" + logic_key)
                .closest(".cool-form__field-group");
            } else {
            ...
            $.each(
              logic_value.logic_data,
              function (conditional_logic_key, conditional_logic_values) {
                if (conditional_logic_values.cfef_logic_field_id) {
                  var value_id = getFieldEnteredValue(
                    conditional_logic_values.cfef_logic_field_id,
                    form
                  );
                  conditionPassFail.push(
                    checkFieldLogic(
                      value_id,
                      conditional_logic_values.cfef_logic_field_is,
                      conditional_logic_values.cfef_logic_compare_value
                    )
                  );
                }
              }
            ...
            var conditionResult =
              fireAction == "All"
                ? conditionPassFail.every(function (fvalue) {
                    return fvalue === true;
                  })
                : conditionPassFail.some(function (fvalue) {
                    return fvalue === true;
                  });
            if (displayMode == "show") {
              if (!conditionResult) {
          ...
          $.each(logicData, function (logic_key, logic_value) {
            if ($(form).find(".is-field-group-" + logic_key).length) {
              ...
              if (
                $(form)
                  .find(".is-field-group-" + logic_key)
                  .hasClass("is-field-type-step")
              ) {
                setTimeout(() => {
                  $(form)
                    .find(".is-field-group-" + logic_key)
                    .find(".cool-form__submit-group")
                    .find(".is-field-type-next, .cool-form__submit-group")
                    .find(".cool-form__button")
                    .attr("id", "form-field-" + logic_key)
                    .closest(".is-field-type-next, .cool-form__submit-group")
    ...
    function performFieldLogic(field, logic_value, form, logic_key, formId) {
      ...
      $.each(logic_value.logic_data, function (conditional_logic_key, conditional_logic_values) {
        var dependent_fi = $(
          ".is-field-group-" + conditional_logic_values.cfef_logic_field_id,
          form
        );
        if (
          dependent_fi.hasClass("is-field-group-acceptance") ||
          dependent_fi.hasClass("is-field-type-acceptance")
        ) {
      ...
      if (displayMode == "show") {
        ...
        if (conditionResult) {
          if (field.hasClass("cfef-step-field")) {
            ...
            var nextButtonText = container.find('button[id^="form-field-"]').text().trim();
    ...
    function logicFixedRequiredShow(formField, file_types, status) {
      if (
        formField.hasClass("is-field-type-radio") &&
        ...
        formField.hasClass("is-field-type-url") &&
        ...
        formField.hasClass("is-field-type-email") &&
        ...
        formField.hasClass("is-field-type-number") &&
        ...
        let value = formField.find("textarea")[0].innerHTML
          ? formField.find("textarea")[0].innerHTML
          : "";
        ...
        if (selectBox.length > 0 && selectBox.find("option").length > 0) {
          ...
          if (selectedValue == "premium1@" || selectedValue[0] == "premium1@") {
            ...
            let value = $(selectedOption).attr("value")
              ? $(selectedOption).attr("value")
              : selectBox.find("option:first").val();
            selectBox.val(value);
          }
        }
      } else {
    ...
    $("body").on(
      "input change",
      ".cool-form input, .cool-form select, .cool-form textarea",
      function (e) {
        var form = $(this).closest(".elementor-widget-cool-form");
        var formId = form.closest(".elementor-element").attr("data-id");
        form.attr("data-form-id", "form-" + formId);
