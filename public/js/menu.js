function UCAccordionMenu(menuID) {
  ...
  function collapseAllExpanded() {
    ...
    if (objAllExpanded.length == false) return false;
  ...
  function toggleSection(objLink) {
    ...
    if (isCollapsed) {
      ...
      if (g_isCloseOnOpen == true) collapseAllExpanded();
  ...
  function openCloseItem(link, event) {
    ...
    if (objSection.hasClass("sub-menu") == false) return true;
  ...
  function expandCurrentItemTree() {
    ...
    if (objCurrentItem.length == 0) return false;
    ...
    if (objParents.length == 0) return false;
